...
// Export a listener for real-time enrollment updates
export function listenToEnrollments(userId, callback) {
    if (!userId) return null;
    
    const enrollmentsRef = collection(db, 'enrollments');
    const q = query(enrollmentsRef, where('userId', '==', userId));
    
    // We'll use onSnapshot for real-time updates
    return null; // Returning null for now, we'll implement this properly
}