...
function buildFallbackDashboardModel(uid, userData = {}) {
    const name = userData.displayName || userData.name || 'Learner';
    return {
        user: userData,
        enrollments: [],
        courses: [],
        stats: {
            ...
            completedCourses: 0,
            totalXp: 0,
            streak: Number(userData.learningStreak) || 0,
            certificates: Number(userData.certificatesCount) || 0
        },
        welcome: {
            title: 'Welcome back',
            name,
            subtitle: 'Your learning dashboard is ready. Start a course and it will appear here.'
        },
        emptyState: {
            title: 'No learning activity yet',
            message: 'Your dashboard will populate as soon as you start learning.'
        }
    };
}
...
async function fetchDashboardData(uid) {
    ...
    try {
        ...
        const fallbackModel = buildFallbackDashboardModel(uid);
        setCachedDashboard(uid, fallbackModel);
